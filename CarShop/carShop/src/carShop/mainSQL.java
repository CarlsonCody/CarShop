...
public class mainSQL extends JFrame{
	...
	static final String netID = "blankenhld04";
    static final String passwordSQL = "lb6990";
	static final String databasePrefix = "cs366-2231";
	...
	public String[][] searchCars(String model, String year, String price, String make) {
		...
		try {
			...
			rs = stmt.executeQuery("SELECT m.make, c.model, c.year, s.price"
					+ " FROM Car c"
					+ " INNER JOIN Manufacturer m ON (c.model = m.model)"
					+ " INNER JOIN Stats s ON (c.model = s.model AND c.year = s.year AND c.price = s.price)"
					+ sb);
